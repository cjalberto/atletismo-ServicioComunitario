	...
	.post('/gestionar/atleta/modificar', (req, res , next) => {
		...
		req.getConnection((err , conexion) => {
			if (err){
				...
				conexion.query(`SELECT  atleta.cedula , atleta.primer_apellido , atleta.primer_nombre , DATE_FORMAT(atleta.fecha_nacimiento,'%Y-%m-%d') fecha, atleta.sexo , club.nombre nombre_club FROM atleta LEFT JOIN club ON atleta.id_club=club.id  WHERE atleta.id = ?` , atleta_id , (err, rows) =>{
	            	if (err){
						res.render('error', {mensaje : 'Error al consultar la base de datos' , code : 404})
					}
	            	else{
