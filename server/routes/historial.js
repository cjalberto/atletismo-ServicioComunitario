	...
	.get('/historial/detalle-competencia/:id', (req, res , next) => {
		...
		req.getConnection((err , conexion) => {
			...
			let promesa = new Promise((resolve , reject) => {
				conexion.query(`SELECT com.nombre as 'nombreCompetencia' , DATE_FORMAT(com.fecha,'%d/%m/%Y') as 'fecha' , TIME(com.hora) as 'hora' , com.lugar as 'lugar' , CONCAT(cat.nombre , '-' , cat.sexo) as 'categoria' FROM competencia com LEFT JOIN categoria cat ON com.id_categoria=cat.id where com.id = ?` , competencia_id, (err , rows) =>{
